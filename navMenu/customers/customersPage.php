...
function deleteData(confirmation,id) {
  if (confirmation == 'yes') {
	window.location="/BillingApplication/navMenu/customers/deleteCustomer?customerId="+id;
  }
}
</script>
<script>
...
<div class="container content">
	<main>
	   <div class="row">
		<div class="col-sm-12">
				<a href="/BillingApplication/navMenu/customers/addCustomersPage.php">
						<button class="btn btn-info"><i class="bx bx-plus"></i>Add customer</button>
					</a>
				</div>
			</div>
		<br>
				...
				<table id="table" class="table table-striped" style="width:100%">
					...
					if (mysqli_num_rows($result) > 0) 
					{
						while($row = mysqli_fetch_assoc($result)) 
            			{
							...
							echo 
							"<td>
								<a href='/BillingApplication/navMenu/customers/editCustomersPage?customerId=".$row["customer_id"]."'>
									...
									<div class="modal-dialog modal-fullscreen-sm-down">
										<div class="modal-content">
											...
											<div class="modal-footer">
												<button type="button" class="btn btn-success" data-bs-dismiss="modal" onclick="deleteData('yes','<?php echo $row['customer_id']?>')">Yes</button>
